...
/*   Updated: 2021/09/15 13:57:34 by aalcara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/header.h"

static void	redir_output(char *filename, int flags)
{
	int	fd_file;
...
void	check_redirects(t_token *current, t_token *end, int *save_std_fd)
{
	while (current != end)
	{
		...
		if (current->type == T_OREDIRECT && current->next->type == T_WORD)
			redir_output(current->next->value, O_WRONLY | O_CREAT | O_TRUNC);
			...
			redir_output(current->next->value, O_WRONLY | O_CREAT | O_APPEND);
